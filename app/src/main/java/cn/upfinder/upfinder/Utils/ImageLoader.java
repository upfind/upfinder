...
import cn.upfinder.upfinder.Libcore.DiskLruCache;
import cn.upfinder.upfinder.MyApplication;
...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = FileUtil.getDiskCacheDir(MyApplication.getContext(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, AppUtil.getAppVersion(MyApplication.getContext()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
